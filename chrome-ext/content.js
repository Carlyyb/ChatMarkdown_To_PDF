(() => {
  ...
  async function performExport(action, messageEl, menuEl) {
    ...
    if (action === 'pdf') {
      ...
      if (markdown) {
        try {
          await exportPdfViaIframe(markdown, messageEl, false, fileName);
          log('PDF 导出完成', { usedMarkdown: true, viaIframe: true });
          return;
        } catch (error) {
          warn('iframe PDF 导出失败，使用 fallback', error);
        }
      }
      ...
      if (!canvas) {
        canvas = await renderDomToCanvas(messageEl, false);
      }

      await saveCanvasAsPdf(canvas, fileName);
      log('PDF 导出完成', { usedMarkdown, viaIframe: false });
      return;
    }
  ...
  function buildMarkdownDocument(markdown) {
    const html = markdownToHtml(markdown || '');
    return `<!DOCTYPE html><html><head><meta charset="utf-8">${MARKDOWN_STYLE}</head><body>${html}</body></html>`;
  }
  ...
  async function renderMarkdownToCanvas(markdown) {
    ...
    const htmlDoc = buildMarkdownDocument(markdown);
  ...
  async function exportPdfViaIframe(markdown, messageEl, exportSelection, fileName) {
    if (!markdown || exportSelection) {
      throw new Error('缺少可用于导出的 Markdown');
    }
    const timeoutMs = 8000;
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('iframe 导出超时')), timeoutMs);
    });
    return Promise.race([
      _exportPdfViaIframeCore(markdown, messageEl, exportSelection, fileName),
      timeoutPromise
    ]);
  }
  ...
  async function _exportPdfViaIframeCore(markdown, messageEl, exportSelection, fileName) {
    ...
    iframe.style.cssText = 'position:fixed;left:0;top:0;width:960px;height:10px;opacity:0;pointer-events:none;border:0;z-index:-1;';
    iframe.srcdoc = buildMarkdownDocument(markdown);
    document.body.appendChild(iframe);

    try {
      await new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error('iframe 加载失败')), 3000);
      ...
      const body = doc.body;
      if (!body) throw new Error('iframe 内容为空');

      const ifw = await loadLibsInIframe(doc);
      log('iframe 库就绪', {
        hasH2C: typeof ifw.html2canvas === 'function',
        hasJsPDF: Boolean(ifw.jspdf?.jsPDF || ifw.jsPDF)
      });
      ...
      if (doc.fonts?.ready) {
        try { await doc.fonts.ready; } catch (error) { warn('iframe 字体等待失败', error); }
      }
      await new Promise((resolve) => setTimeout(resolve, 60));

      const canvas = await ifw.html2canvas(body, {
        backgroundColor: '#ffffff',
        scale: 2,
        useCORS: true,
        allowTaint: true,
      ...
      if (!canvas || canvas.width === 0 || canvas.height === 0) {
        throw new Error('iframe 渲染结果为空');
      }

      const JsPDFCtor = ifw.jspdf?.jsPDF || ifw.jsPDF;
      await saveCanvasAsPdf(canvas, fileName, JsPDFCtor);
    } finally {
      iframe.remove();
    }
  }
  ...
  async function loadLibsInIframe(doc) {
    ...
    const getURL = (path) => (typeof chrome !== 'undefined' && chrome.runtime?.getURL
      ? chrome.runtime.getURL(path)
      : path);

    async function inlineScript(url) {
      const response = await fetch(getURL(url), { cache: 'no-store' });
      if (!response.ok) {
        throw new Error(`fetch 失败: ${url}`);
      }
    ...
    if (ifw.jspdf?.jsPDF && !ifw.jsPDF) {
      try { ifw.jsPDF = ifw.jspdf.jsPDF; } catch (error) { warn('jsPDF 桥接失败', error); }
    }

    for (let i = 0; i < 50; i += 1) {
      const readyH2C = typeof ifw.html2canvas === 'function';
      const readyJsPDF = Boolean(ifw.jspdf?.jsPDF || ifw.jsPDF);
      if (readyH2C && readyJsPDF) {
        return ifw;
      }
  ...
  async function saveCanvasAsPdf(canvas, fileName, JsPDFCtorOverride) {
    const JsPDFCtor = JsPDFCtorOverride || (window.jspdf && window.jspdf.jsPDF) || window.jsPDF;
